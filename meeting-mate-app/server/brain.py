...
回答の品質を上げるために、追加で別のツールを使うべきか判定してください。
...
## 判定ルール（以下の1つでも該当すれば "needed"）
1. 回答に「データがない」「アクセスできない」「確認が必要」「ご共有いただければ」「お伝えできない」等の記述があり、利用可能ツールで補完できる
2. 利用者に情報の提供を求めているが、利用可能ツールで取得できる可能性がある
3. 元のリクエストの一部に対する回答が推測・仮定に基づいている
4. 元のリクエストの一部が明確に未回答

## "none" にする条件（すべて満たす場合のみ）
- 元のリクエストのすべての部分に具体的データで回答済み
- 利用可能ツールで追加取得できる有益な情報がない
- deep_analysis は重い処理なので、本当に必要な場合のみ選択
...
# ================================================================
# Follow-up Tool Descriptions
# ================================================================

_TOOL_DESCRIPTIONS = {
    "knowledge_base_search": '社内データベース検索 — args: {{ "query": "検索キーワード", "category": "sales|policies|projects|general" }}',
    "calculate": '数値計算 — args: {{ "expression": "数式", "description": "計算の説明" }}',
    "get_current_time": "現在時刻の取得 — args: {}",
    "get_meeting_context": "会議情報の整理 — args: {}",
    "summarize_discussion": '議論を要約 — args: {{ "focus": "all|decisions|issues|actions" }}',
    "create_task": 'タスク登録 — args: {{ "title": "タスク名", "assignee": "担当者", "due_date": "YYYY-MM-DD", "priority": "high|medium|low" }}',
    "generate_diagram": 'Mermaid図生成 — args: {{ "description": "図の説明", "diagram_type": "flowchart|sequence|gantt|mindmap|pie" }}',
    "deep_analysis": '深層分析（複雑な分析・最新情報・専門知識） — args: {{ "question": "分析対象の質問" }}',
}


def _get_available_tools_description(exclude: set[str] | None = None) -> str:
    """Follow-up 判定用: 使用済みツールを除外したツール一覧を返す"""
...
async def process_brain_request(request: str, meeting_context: dict) -> dict:
    ...
    if tool_name == "deep_analysis":
        ...
        if tool_result.get("routed") and tool_result.get("analysis"):
            ...
            # Follow-up: deep_analysis 後に社内データで補完できるかチェック
            all_tool_results = [{"tool": tool_name, "args": tool_args, "result": {"analysis": response_text[:500]}}]
            ...
            for follow_up_i in range(MAX_FOLLOW_UPS):
                ...
                fu_decision = None
                try:
                    available_tools = _get_available_tools_description(exclude=used_tools)
                    ...
                    logger.debug(f"[Brain] Deep follow-up raw: {fu_raw[:300]}")
                except Exception as e:
                    logger.warning(f"[Brain] Deep analysis follow-up failed: {e}")

                t_fu_judge_end = time.perf_counter()
                steps.append({
                    "id": f"follow_up_judge_{follow_up_i + 1}",
                    "label": "追加情報判定",
                    "model": BRAIN_LLM_MODEL,
                    "elapsed_ms": round((t_fu_judge_end - t_fu_start) * 1000),
                })

                if fu_decision is None or fu_decision.get("follow_up") != "needed":
                    logger.info(f"[Brain] Deep follow-up #{follow_up_i + 1}: not needed")
                    break

                fu_tool = fu_decision.get("tool", "")
                fu_args = fu_decision.get("args", {})
                logger.info(f"[Brain] Deep follow-up #{follow_up_i + 1}: {fu_tool}({fu_args})")
                used_tools.add(fu_tool)
                ...
                try:
                    ...
                    logger.error(f"[Brain] Deep follow-up tool failed: {e}")
                    break
                t_fu_exec_end = time.perf_counter()

                all_tool_results.append({"tool": fu_tool, "args": fu_args, "result": fu_result})
                actions.extend(extract_actions(fu_tool, fu_result))

                steps.append({
                    "id": f"follow_up_exec_{follow_up_i + 1}",
                    "label": f"追加ツール実行 ({fu_tool})",
                    "model": None,
                    "elapsed_ms": round((t_fu_exec_end - t_fu_exec_start) * 1000),
                })

                # 統合応答生成: deep_analysis の結果 + 追加ツールの結果
                t_fu_resp_start = time.perf_counter()
                try:
                    response_text = await llm_complete_with_tools(
                        ...
                        prompt=RESPONSE_GENERATION_PROMPT.format(
                            ...
                            tool_result=json.dumps(
                                [{"tool": r["tool"], "result": r["result"]} for r in all_tool_results],
                                ensure_ascii=False,
                            ),
                            context_summary=context_summary,
                        ),
                    ...
                    logger.error(f"[Brain] Deep follow-up response failed: {e}")
                t_fu_resp_end = time.perf_counter()

                steps.append({
                    "id": f"follow_up_response_{follow_up_i + 1}",
                    "label": "統合応答生成",
                    "model": BRAIN_LLM_MODEL,
                    "elapsed_ms": round((t_fu_resp_end - t_fu_resp_start) * 1000),
                })
            ...
            return {
                ...
                "metadata": {
                    ...
                    "follow_up_count": len(all_tool_results) - 1,
                    "steps": steps,
...
会議コンテキスト: {context_summary}""",
    ...
    # Pass 2: 応答テキスト生成
    all_tool_results = [{"tool": tool_name, "args": tool_args, "result": tool_result}]
    ...
    steps = [
        ...
        },
        {
            ...
            "label": "応答生成",
            "model": BRAIN_LLM_MODEL,
            "elapsed_ms": round((t_resp_end - t_resp_start) * 1000),
        },
    ]

    # Pass 3: Follow-up 判定ループ（最大2回）
    used_tools = {tool_name}
    ...
    for follow_up_i in range(MAX_FOLLOW_UPS):
        ...
        try:
            ...
            fu_json = strip_code_blocks(fu_raw)
            fu_decision = json.loads(fu_json)
            logger.debug(f"[Brain] Follow-up raw: {fu_raw[:300]}")
        except Exception as e:
            logger.warning(f"[Brain] Follow-up judgment failed: {e}, raw={fu_raw[:200] if 'fu_raw' in dir() else 'N/A'}")
        ...
        if fu_decision is None or fu_decision.get("follow_up") != "needed":
            logger.info(f"[Brain] Follow-up #{follow_up_i + 1}: not needed (decision={fu_decision})")
            break

        fu_tool = fu_decision.get("tool", "")
        fu_args = fu_decision.get("args", {})
        logger.info(f"[Brain] Follow-up #{follow_up_i + 1}: {fu_tool}({fu_args})")
        used_tools.add(fu_tool)
        ...
        try:
            ...
            logger.error(f"[Brain] Follow-up tool execution failed: {e}")
            break
        t_fu_exec_end = time.perf_counter()
        ...
        try:
            ...
            logger.error(f"[Brain] Follow-up response generation failed: {e}")
        t_fu_resp_end = time.perf_counter()
