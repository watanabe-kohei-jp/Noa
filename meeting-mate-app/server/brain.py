...
MAX_FOLLOW_UPS = 2
# Follow-up 対象外のツール（早期 return や再帰リスクがあるもの）
_FOLLOW_UP_EXCLUDED_TOOLS = {"direct_response", "deep_analysis", ""}
...
会議コンテキスト: {context_summary}""",
    ...
    for follow_up_idx in range(MAX_FOLLOW_UPS):
        ...
        if fu_tool in _FOLLOW_UP_EXCLUDED_TOOLS:
            logger.info(f"[Brain] Follow-up #{follow_up_idx + 1}: excluded tool '{fu_tool}', stopping")
            break
    ...
    # Pass 2: 応答テキスト生成（全ツール結果を使用）
    combined_tool_result = json.dumps(tool_history, ensure_ascii=False) if len(tool_history) > 1 else json.dumps(tool_result, ensure_ascii=False)
