...
FOLLOW_UP_PROMPT = """あなたは会議AIアシスタント「Noa」のブレインです。
...
# ================================================================
# Tool Handlers (extracted from execute_tool if/elif chain)
# ================================================================
...
def _register_builtin_tools() -> None:
    """全 builtin ツールを registry に登録する"""
    from integrations.google_calendar import handle_google_calendar_create
...
# Follow-up で許可するツール（registry から導出）
_FOLLOW_UP_ALLOWED_TOOLS = _registry.get_follow_up_allowed()
...
def extract_actions(tool_name: str, tool_result: dict) -> list[dict]:
    ...
    if tool_name == "google_calendar_create" and tool_result.get("success"):
        actions.append({
            ...
            "data": {
                "calendar_url": tool_result.get("calendar_url", ""),
                "event_summary": tool_result.get("event_summary", ""),
...
async def process_brain_request(request: str, meeting_context: dict) -> dict:
    ...
    try:
        tool_selection_raw = await llm_complete(
            ...
            prompt=TOOL_SELECTION_PROMPT.format(
                tool_list=_registry.build_tool_prompt(),
                request=request,
    ...
    # NOTE: 特別フロー — 通常のツール実行 + 応答生成パスをバイパスする
...
会議コンテキスト: {context_summary}""",
    ...
    for follow_up_idx in range(MAX_FOLLOW_UPS):
        ...
        try:
            fu_raw = await llm_complete(
                ...
                prompt=FOLLOW_UP_PROMPT.format(
                    ...
                    available_tools=_registry.build_follow_up_tool_list(),
                ),
                api_key=api_key,
                temperature=0.1,
                max_tokens=500,
        ...
        if fu_tool not in _FOLLOW_UP_ALLOWED_TOOLS:
